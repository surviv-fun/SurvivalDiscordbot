...
/**
 * SurvivalDiscordbot; fun.surviv.discord.cli.command:CLICommandMap
 *
 * @author LuciferMorningstarDev - https://github.com/LuciferMorningstarDev
 * @since 29.08.2022
 */
public class CLICommandMap extends HashMap<String, CLICommandExecutor> {

    public CLICommandMap(CLICommandExecutor... initialCommands) {
        for (CLICommandExecutor cmd : initialCommands) {
            addCommand(cmd);
        }
    }

    public void addCommand(CLICommandExecutor command) {
        this.put(command.name(), command);
